...
const DecisionMakingReporting = () => {
  ...
  // Fetch data with real API calls but graceful fallback
  const fetchData = async () => {
    setIsLoading(true);
    try {
      // Attempt real API calls - will fail gracefully if no backend
      const [candidatesResponse, analyticsResponse] = await Promise.all([
        fetch('/api/candidates').catch(() => ({ ok: false })),
        fetch('/api/decision-analytics').catch(() => ({ ok: false }))
      ]);

      // If either call succeeds, use that data, otherwise use defaults
      const candidatesData = candidatesResponse.ok ? 
        await candidatesResponse.json().catch(() => DEFAULT_CANDIDATES) : 
        DEFAULT_CANDIDATES;

      const analyticsData = analyticsResponse.ok ? 
        await analyticsResponse.json().catch(() => DEFAULT_ANALYTICS) : 
        DEFAULT_ANALYTICS;
  ...
  // Handle actions with attempted API calls
  const handleAction = async (index, action) => {
    try {
      // Attempt to call API but don't wait for success
      fetch('/api/candidates/status', {
        method: 'PUT',
  ...
  // Send report with attempted API call
