...
// New component to display skills threshold data
const SkillsThresholdDashboard = ({ data }) => {
  ...
  const renderItemRow = (item, data) => (
    <div key={item} className="border-b border-gray-200 py-3">
      ...
      <div className="grid grid-cols-3 gap-4 mt-2">
        <div>
          ...
          {renderScoreBar(data.importance, 'bg-blue-500')}
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">
            <span>Selection</span>
            <span>{data.selection_score}%</span>
          </div>
          {renderScoreBar(data.selection_score, 'bg-green-500')}
        </div>
        <div>
          <div className="flex justify-between text-xs mb-1">
            <span>Rejection</span>
            <span>{data.rejection_score}%</span>
          </div>
          {renderScoreBar(data.rejection_score, 'bg-red-500')}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="bg-white shadow-md rounded-lg p-4 mb-4">
      <h3 className="text-lg font-semibold mb-3">Skills Threshold Analysis for {role}</h3>
      
      <div className="flex justify-between items-center mb-4">
        <div className="flex space-x-2">
          <button 
            onClick={() => setActiveTab('skills')}
            className={`px-3 py-1 rounded ${activeTab === 'skills' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          >
            Skills
          </button>
          <button 
            onClick={() => setActiveTab('achievements')}
            className={`px-3 py-1 rounded ${activeTab === 'achievements' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          >
            Achievements
          </button>
          <button 
            onClick={() => setActiveTab('activities')}
            className={`px-3 py-1 rounded ${activeTab === 'activities' ? 'bg-blue-600 text-white' : 'bg-gray-200'}`}
          >
            Activities
          </button>
        </div>
        ...
        <div className="flex space-x-4">
          <div className="bg-blue-100 px-3 py-1 rounded-full text-sm">
            ...
            <span className="ml-1 font-bold text-green-700">{data.selection_threshold.toFixed(3)}</span>
          </div>
          <div className="bg-red-100 px-3 py-1 rounded-full text-sm">
            <span className="font-medium">Rejection Threshold:</span>
            <span className="ml-1 font-bold text-red-700">{data.rejection_threshold.toFixed(3)}</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4">
        {activeTab === 'skills' && (
          <div>
            <h4 className="font-medium text-gray-700 mb-3">Core Skills</h4>
            <div className="space-y-1">
              {Object.entries(roleData.skills).map(([skill, skillData]) => 
                renderItemRow(skill, skillData)
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'achievements' && (
          <div>
            <h4 className="font-medium text-gray-700 mb-3">Achievements & Certifications</h4>
            <div className="space-y-1">
              {Object.entries(roleData.achievements).map(([achievement, achievementData]) => 
                renderItemRow(achievement, achievementData)
              )}
            </div>
          </div>
        )}
        ...
        {activeTab === 'activities' && (
          <div>
            <h4 className="font-medium text-gray-700 mb-3">Skilled Activities</h4>
            <div className="space-y-1">
              {Object.entries(roleData.activities).map(([activity, activityData]) => 
                renderItemRow(activity, activityData)
              )}
            </div>
          </div>
        )}
...
const JDTab = ({ jdList, setJdList, setSelectedJDForSidebar }) => {
  ...
  const buttonConfig = {
    ...
    'Threshold Settings': { icon: <FaChartBar className="mr-2 text-orange-500" />, color: 'orange' },
  };
  ...
  const handleOpenThresholdSettings = () => {
    // Example threshold data - in a real app, you would fetch this from your API
    const sampleThresholdData = {
      ...
      "selection_threshold": 14.075,
      "rejection_threshold": 14,
      "status": "success",
      "selected_prompts": [
        "Adjust WordPress Site Support's rejection score from 20.0% to 30.0%",
        "Adjust Independent Work's rejection score from 20.0% to 30.0%",
        "Change Social Media Analytics Reporting's importance from 25.0% to 30.0%",
        "Set Undergraduate Degree (Any Discipline)'s selection score from 100.0% to 100.0%",
        "Update Training Receptiveness's rating from 4.0 to 5.0"
      ]
    };
    
    setThresholdData(sampleThresholdData);
    setShowAnalysisModal(true);
      ...
      <div className="flex justify-between items-center mb-0 mt-8">
        ...
        <div className="flex gap-4">
          ...
          <button
            ...
            className="p-2 text-gray-700 rounded hover:bg-gray-100 flex items-center"
          >
            {buttonConfig['Threshold Settings'].icon}
            Threshold Settings
          </button>
        </div>
      </div>
          ...
          <div className="bg-white rounded-lg shadow-lg p-6 w-11/12 max-w-4xl max-h-screen overflow-y-auto">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold">
                {thresholdData ? "Threshold Settings" : "Resume & Job Description Analysis"}
              </h2>
              <button 
            ...
            {thresholdData ? (
              // Render threshold settings dashboard
              <SkillsThresholdDashboard data={thresholdData} />
            ) : !showDashboards ? (
              <div className="mb-6">
