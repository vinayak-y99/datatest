...
export default function CS({ resumeId, selectedFile, resumeData, onClose }) { // Updated props to match ResumeTab
  ...
  // Reset visibility and update file when resumeId or selectedFile changes
  useEffect(() => {
    setIsPageVisible(true); // Reset visibility to true when a new resume is selected
    if (selectedFile) {
      setUploadedFile(selectedFile);
      handleAnalysisComplete({
        file: selectedFile,
        id: resumeId,
        type: 'resume', // Changed to 'resume' to reflect context
        fileName: selectedFile.name // Added fileName to match ResumeTab
      });
    }
  }, [resumeId, selectedFile]); // Updated dependencies to match props

  const handleAnalysisComplete = (data) => {
    console.log(`Analysis data received for resume ${resumeId}:`, data);
  ...
  const handleCreate = () => {
    console.log(`Creating dashboard for resume ${resumeId} with analysis data:`, analysisData);
              ...
              <div className="flex relative">
                ...
                <div style={{ width: '100%', clear: 'both' }}>
                  <span style={{ paddingLeft: '34px', fontSize: '20px' }}>
                    {uploadedFile && uploadedFile.name ? `Resume: ${uploadedFile.name}` : `Resume ${resumeId}`} {/* Updated to show resume file name or ID */}
                  </span>
