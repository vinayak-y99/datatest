...
export default function CollaborationBoard() {
  ...
  useEffect(() => {
    ...
    const handleMockNotifications = () => {
      ...
      switch(randomNotification.type) {
        case 'interview':
          addNotification(`Interview scheduled: ${randomNotification.candidate} for ${randomNotification.role} position on ${randomNotification.date} at ${randomNotification.time}`);
          break;
        case 'evaluation':
          addNotification(`Evaluation needed: ${randomNotification.candidate} (${randomNotification.role}) - Awaiting review from ${randomNotification.interviewer}`);
          break;
        case 'decision':
          addNotification(`${randomNotification.candidate} - ${randomNotification.role}: ${randomNotification.status}`);
          break;
      }
    };

    handleMockNotifications();
    const interval = setInterval(handleMockNotifications, 5000);
  ...
  const submitEvaluation = () => {
    ...
    if (userRole === "recruiter" || userRole === "technical-panel") {
      ...
      socket.emit("newEvaluation", newEvaluation);
      setMessage("");
    } else {
