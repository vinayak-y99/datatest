...
const MeetingQAConverterPreview = ({ onGenerateQA, onEvaluationUpdate }) => {
  ...
  //         try {
  //             const initResponse = await fetch('http://localhost:8000/api/v1/jd/initialize', {
  //                 method: 'POST',
  ...
  const handleMapAnswer = async () => {
      try {
          const response = await fetch('http://localhost:8000/api/v1/jd/evaluate-qa', {
              method: 'POST'
          });
  ...
  const handleClear = async () => {
      try {
          const response = await fetch('http://localhost:8000/api/v1/jd/clear-transcription', {
              method: 'POST'
          });
  ...
  const generateQAPairs = async () => {
      ...
      try {
          const response = await fetch('http://localhost:8000/api/v1/jd/generate-qa-from-audio', {
              method: 'POST',
              headers: {
